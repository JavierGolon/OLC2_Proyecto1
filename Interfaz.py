...
class Notepad: 
	...
	__framebotton = Frame(__root,width=__root.winfo_screenwidth() , height = 250, background="#084B8A")
	# Creamos la pestaña
	...
	def __openFile(self): 
		...
		if self.__file == "": 
			...
			self.__mytabs.tab(self.__mytabs.select(),text=os.path.basename(self.__file)) 
	...
	def __Search(self):
		countVar = StringVar()
		pos = self.__GetSelectedText().search("a", "1.0", stopindex="end", count=countVar)
